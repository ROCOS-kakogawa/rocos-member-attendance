...
function mergePieceTasks(defaults, saved = []) {
  ...
  return merged.concat(saved.filter((item) => !defaults.some((task) => task.key === item.key)));
}
...
function renderTaskSettings() {
  ...
  els.taskSettings.querySelectorAll("[data-add-task]").forEach((button) => {
    button.addEventListener("click", handleAddTaskSetting);
  });
  els.taskSettings.querySelectorAll("[data-delete-task]").forEach((button) => {
    button.addEventListener("click", handleDeleteTaskSetting);
  });
}
    ...
    <section class="task-settings-group">
      ...
      ${state.taskRates[group].map((task, index) => `
        <div class="task-setting-row">
          ...
          <button class="delete-task-button" type="button" data-delete-task data-group="${group}" data-index="${index}" title="削除">削除</button>
        </div>
      `).join("")}
      <button class="add-task-button" type="button" data-add-task data-group="${group}">＋ 項目を追加</button>
    </section>
  `;
}
    ...
    <section class="task-settings-group">
      ...
      ${state.taskRates.piece.map((task) => `
        <div class="task-setting-row">
          ...
          <button class="delete-task-button" type="button" data-delete-task data-group="piece" data-index="${task.key}" title="削除">削除</button>
        </div>
      `).join("")}
      <button class="add-task-button" type="button" data-add-task data-group="piece">＋ 項目を追加</button>
    </section>
  `;
}
...
function handleAddTaskSetting(event) {
  const group = event.target.dataset.group;
  if (group === "piece") {
    state.taskRates.piece.push({
    ...
    state.taskRates[group].push({ name: "新しい項目", amount: 0 });
  }
  saveState();
  renderTaskSettings();
  renderAttendanceTable();
...
function handleDeleteTaskSetting(event) {
  const { group, index } = event.target.dataset;
  const taskName = group === "piece"
    ? ((state.taskRates.piece.find((item) => item.key === index) || {}).name || "この項目")
    : ((state.taskRates[group][Number(index)] || {}).name || "この項目");
  if (!confirm(`${taskName}を削除しますか？ 入力済みのこの項目の加算も消えます。`)) return;

  if (group === "piece") {
    state.taskRates.piece = state.taskRates.piece.filter((task) => task.key !== index);
    removePieceTaskData(index);
  } else {
    const taskIndex = Number(index);
    state.taskRates[group].splice(taskIndex, 1);
    removeIndexedTaskData(group, taskIndex);
  }
...
function removeIndexedTaskData(group, removedIndex) {
  Object.keys(state.taskCounts || {}).forEach((userId) => {
    if (state.taskCounts[userId] && state.taskCounts[userId][group]) {
      state.taskCounts[userId][group] = shiftIndexedValues(state.taskCounts[userId][group], removedIndex);
    }
  });

  Object.keys(state.taskDaily || {}).forEach((key) => {
    if (state.taskDaily[key] && state.taskDaily[key][group]) {
      state.taskDaily[key][group] = shiftIndexedValues(state.taskDaily[key][group], removedIndex);
    }
  });

  if (group === "monthly") {
    Object.keys(state.taskMonthly || {}).forEach((month) => {
      state.taskMonthly[month] = shiftIndexedValues(state.taskMonthly[month], removedIndex);
    });
  }
}

function removePieceTaskData(taskKey) {
  Object.keys(state.taskCounts || {}).forEach((userId) => {
    if (state.taskCounts[userId] && state.taskCounts[userId].piece) {
      delete state.taskCounts[userId].piece[taskKey];
    }
  });

  Object.keys(state.taskDaily || {}).forEach((key) => {
    if (state.taskDaily[key] && state.taskDaily[key].piece) {
      delete state.taskDaily[key].piece[taskKey];
    }
  });
}

function shiftIndexedValues(values, removedIndex) {
  return Object.keys(values || {}).reduce((result, key) => {
    const numericKey = Number(key);
    if (numericKey < removedIndex) result[key] = values[key];
    if (numericKey > removedIndex) result[String(numericKey - 1)] = values[key];
    return result;
  }, {});
}
